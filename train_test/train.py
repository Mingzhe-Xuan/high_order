...
def train(
    ...
    ehull_trainset, ehull_valset, ehull_testset = (
        get_scalar_dataloaders_split(
    ...
    bandgap_trainset, bandgap_valset, bandgap_testset = (
        get_scalar_dataloaders_split(
    ...
    e_form_trainset, e_form_valset, e_form_testset = (
        get_scalar_dataloaders_split(
    ...
    elastic_total_kbar_trainset, elastic_total_kbar_valset, elastic_total_kbar_testset = (
        get_tensor_dataloaders_split(
    ...
    piezoelectric_C_m2_trainset, piezoelectric_C_m2_valset, piezoelectric_C_m2_testset = (
        get_tensor_dataloaders_split(
    ...
    piezoelectric_e_Angst_trainset, piezoelectric_e_Angst_valset, piezoelectric_e_Angst_testset = (
        get_tensor_dataloaders_split(
    ...
    # 3. shared equivariant layers for self train and tensor train
    # 4. different readout layers for each property in scalar train and tensor train
    embedding_layer = EmbeddingLayer(dist_emb_func, embed_dim, max_atom_type, cutoff)
    invariant_layers = nn.ModuleList([InvariantLayer(inv_update_method, embed_dim) for _ in range(num_inv_layers)])
    irreps_list = [f"{scalar_dim}x0e"]
    l_max = num_equi_layers
    for l in range(1, l_max+1):
        p = "e" if l % 2 == 0 else "o"
        irreps_list.append(f"{vec_dim}x{l}{p}")
