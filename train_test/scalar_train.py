...
def scalar_train(
    ...
    for epoch in range(start_epoch, min(num_epochs, start_epoch + limit)):
        ...
        print(f"Epoch {epoch+1}/{num_epochs}, Avg Loss: {avg_loss:.6f}, Avg MAE: {avg_mae:.6f}")
        ...
        if avg_loss < best_loss:
            ...
            print(f"Saved best model with loss: {best_loss:.6f}, MAE: {avg_mae:.6f}")
    ...
    print(f"Training completed. Best loss: {best_loss:.6f}, Best MAE: {avg_mae:.6f}")
    plot_loss(train_losses, pic_dir, property_name)
    plot_mae(train_mae, pic_dir, property_name)
    return model, train_losses, train_mae
